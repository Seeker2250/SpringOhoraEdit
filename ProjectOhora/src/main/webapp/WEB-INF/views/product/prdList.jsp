		...
		<div id="second-wrap-layout">
			...
			<div id="item-list-wrap">
			                        ...
			                        <li id="itembox${status.index + 1}" class="item-wrap">
			                            <div class="item-container">
			                                <dl>
			                                    ...
			                                    <a href="/projectOhora/product/view.do?product_id=${pdt.pdtId}" class="item-viewlink"></a>
			                                    
			                                    
			                                    <div class="item-image">
			                                        <img src="${pdt.pdtImgUrl}" alt="${pdt.pdtName}" width="800" height="800" />
			                                        <img src="${pdt.pdtImgUrl2}" alt="${pdt.pdtName}" width="800" height="800" />
			                                    </div>
			                                        ...
			                                        <div class="cart-in">
			                                            <c:choose>
			                                                <c:when test="${userId != null}">
			                                                    <img src="../image/btn_list_cart.gif" data-pdtid="${pdt.pdtId}" alt="Add to cart" onclick="addCart(this);" />
			                                                </c:when>
			                                                <c:otherwise>
			                                                    <img src="../image/btn_list_cart.gif" data-pdtid="${pdt.pdtId}" alt="Add to cart" onclick="noUserCart(this);" />
			                                                </c:otherwise>
			                                            </c:choose>
			                                        </div>
			                                    </div>
				...
				<div id="page-container">    
				    <!-- 첫 페이지 링크 -->
				    <a href="?catno=${categoryNumber}&currentPage=1&sort=${param.sort}<c:if test='${param.keyword != null}'>&keyword=${param.keyword}</c:if>" class="first">first</a>            
				
				    <!-- 이전 페이지 링크 -->
				    <c:if test="${pvo.currentPage > 1}">
				        <a href="?catno=${categoryNumber}&currentPage=${pvo.currentPage - 1}&sort=${param.sort}<c:if test='${param.keyword != null}'>&keyword=${param.keyword}</c:if>" class="prev">prev</a>
				    </c:if>                
				    ...
				    <ol>
				        <c:forEach begin="${pvo.start}" end="${pvo.end}" step="1" var="i">
				            <c:choose>                
				                ...
				                <c:otherwise>
				                    <li><a href="?catno=${categoryNumber}&currentPage=${i}&sort=${param.sort}<c:if test='${param.keyword != null}'>&keyword=${param.keyword}</c:if>">${i}</a></li>
				                </c:otherwise>
				            </c:choose>
				        </c:forEach>
				    </ol>                
				    ...
				    <c:if test="${pvo.end < pvo.totalPages}">
				        <a href="?catno=${categoryNumber}&currentPage=${pvo.currentPage + 1}&sort=${param.sort}<c:if test='${param.keyword != null}'>&keyword=${param.keyword}</c:if>" class="next">next</a>
				    </c:if>                
				
				    <!-- 마지막 페이지 링크 -->
				    <a href="?catno=${categoryNumber}&currentPage=${pvo.totalPages}&sort=${param.sort}<c:if test='${param.keyword != null}'>&keyword=${param.keyword}</c:if>" class="last">last</a>
				</div>
